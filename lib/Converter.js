...
class Converter {
    ...
    static _updateObject(field, value, element) {
        ...
        for (const [index, key] of Object.keys(element).entries()) {
            ...
            for (let _index = 0, _len = fields.length; _index < _len; _index++) {
                ...
                if (field.indexOf('.') !== -1) {
                    ...
                    if (fieldFirstKey === key) {
                        let fieldSelection = element[fieldFirstKey];

                        if (Converter._isObject(fieldSelection) === true) {
                            fieldParts.shift();

                            fieldSelection = Converter._updateObject(fieldParts.join('.'), value, fieldSelection);
                        }
                        ...
                        if (Converter._isArray(fieldSelection) === true) {
                            ...
                            for (let __index = 0, __len = fieldSelection.length; __index < __len; __index++) {
                                fieldSelection[__index] = Converter._updateObject(fieldParts.join('.'), value, fieldSelection[__index]);
                            }
                        }
                    }
                }
    ...
    static _getObjectValue(field, data) {
        ...
        for (const [index, key] of Object.keys(data).entries()) {
            ...
            if (field.indexOf('.') !== -1) {
                const fields = field.split('.');
                const fieldFirstKey = fields[0] || null;

                if (fieldFirstKey === key) {
                    let fieldSelection = data[fieldFirstKey];

                    if (Converter._isObject(fieldSelection) === true || Converter._isArray(fieldSelection) === true) {
                        fields.shift();

                        fieldSelection = Converter._getObjectValue(fields.join('.'), fieldSelection);
                        getValue = fieldSelection;
                    }
                }
            }
    ...
    static update(field, value, data) {

        // object
        if (Converter._isObject(data) === true) {
            Converter._updateObject(field, value, data);
            return data;
        }
        ...
        if (Converter._isArray(data) === true) {
            ...
            for (let index = 0, len = data.length; index < len; index++) {
                data[index] = Converter._updateObject(field, value, data[index]);
            }
    ...
    static getObjectValue(field, data) {

        return Converter._getObjectValue(field, data);
    }
}
